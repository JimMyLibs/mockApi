...
export const getRandom = (type) => {// 生成模拟数据: 数值类型模拟: 待处理
    ...
    if(/^cnStr\[\d{1,}\]$/.test(type)){// 20190829：cnStr[100]: 100以内随机个汉字
        const max = type.slice(6,-1)*1;
        const cparagraph = Random.cparagraph();
        if(max<100){
            return cparagraph.slice(0,Math.ceil(Math.random() * max))
        }else{
            return cparagraph.repeat(max/100+1).slice(0,Math.ceil(Math.random() * max)) 
        }        
    }else if(/^enStr\[\d{1,}\]$/.test(type)){// 20190829：cnStr[100]: 100以内随机个汉字
        const max = type.slice(6,-1)*1;
        const paragraph = Random.paragraph();
        if(max<400){
            return paragraph.slice(0,Math.ceil(Math.random() * max))
        }else{
            return paragraph.repeat(max/100+1).slice(0,Math.ceil(Math.random() * max)) 
        }        
    }else if(type == 'idCard'){
        return getIdCard();
    }else if(type == 'icon'){
